function our_plot_components_3D_GUI(Y,A,F,C,F_dff,C_dec,S_dec,b,f,Cn,options)
...
% if plot_df
%     %[~,Df] = extract_DF_F(Y,[A,double(b)],[C;f],[],options);%%This function may lead to negtive Df.
%     [~,F0] = detrend_df_f(A,b,C,f,Y_r-C,options);
% else
%     %Df = ones(size(A,2)+1,1);
%     F0 = ones(size(A,2)+1,1);
% end
% Y_r = Y_r./F0;
% C = C./F0;
...
% For R2014a and earlier: set(f,'Visible','on');
    ...
    function plot_component(i)
        ...
        subplot(3,3,[3,6,9]);
        if i <= K
            plot(1:T,F(i,:),'linewidth',2); hold all; plot(1:T,C(i,:),'linewidth',2);plot(1:T,C_dec(i,:),'linewidth',2);plot(1:T,S_dec(i,:),'linewidth',2);
            if plot_df
            ...
            leg = legend('Raw trace (filtered)','Inferred','Deconvolved','Spike');
        ...
        if i <= K
            figure(23);plot(1:T,F_dff(i,:),'linewidth',2);plot(1:T,C_dec(i,:),'linewidth',2);hold on;plot(1:T,S_dec(i,:),'linewidth',2);
            leg = legend('F dff','Deconvolved','Spike');
            set(leg,'FontSize',14,'FontWeight','bold');
            drawnow;
            hold off;
        end
