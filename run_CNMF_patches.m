...
if memmaped    
    parfor i = 1:n_patches
        ...
        Yp = data.Y(patch_idx{:},:);
        K = 500.0/700000*((patches{i}(2)-patches{i}(1)+1)*(patches{i}(4)-patches{i}(3)+1)*(patches{i}(6)-patches{i}(5)+1)); % set K considering the size of the patch
        K = round(K) + 1; % to avoid too small K
